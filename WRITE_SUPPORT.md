...
commands. `SMT750IC` with UPS firmware `18.0` is temporarily allowlisted for
controlled physical acceptance on a noncritical load. This is not yet a
release-support claim.

## Capability matrix

Every cell also requires an exact hardware-accepted model/UPS-firmware pair,
...
1. Run and archive the contract's noncritical-load physical acceptance record
   for the candidate `SMT750IC`/`18.0` pair. Remove it from the allowlist if it
...
The first release must advertise only exact hardware combinations with a
